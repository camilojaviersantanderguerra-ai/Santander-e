...
async function shopifyFetch<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
  ...
  // DEBUG TEMPORAL: registrar la respuesta cruda para diagnosticar por qué
  // el catálogo vuelve vacío sin error. Quitar esta línea una vez resuelto.
  console.log("SHOPIFY_DEBUG raw response:", JSON.stringify(json).slice(0, 800));
  if (json.errors) {
